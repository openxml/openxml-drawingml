module PropertyTestMacros
  ...
  def xml(obj)
    ...
    doc = Nokogiri::XML::Builder.new do |xml|
      xml.root("xmlns:a" => "http://anamespace.org", "xmlns:r" => "http://rnamespace.org") {
        obj.to_xml(xml)
      }
    end.to_xml
    doc_pattern =~ doc ? $1 : ""
  ...
  module ClassMethods
    ...
    def it_should_use(tag: nil, name: nil, namespace: :a, value: nil)
      context "always" do
        before(:each) do
    ...
    def it_should_output_expected_xml(*values, expected_xml: nil)
      it "should output the correct XML" do
        ...
        expected_xml ||= "<a:#{instance.tag} #{property_namespace}#{property_namespace.nil? ? "" : ":"}#{property_name}=\"#{value}\"/>"
